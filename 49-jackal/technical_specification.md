...
- Migrate ghga-transpiler to a more general configuration strategy that ideally lives longer than individual metadata models
  - Refactor the transpiler to read the infomation on i) the data types (e.g., list, str, int), ii) the value restriction (unrescticted, controlled vocabulary, value from another class) from a hidden sheet in the XLS file. This information will be presented in table form with boolean values.
